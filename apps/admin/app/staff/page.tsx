...
const roleLabels: Record<StaffRole, { ar: string; en: string; workspace: string; level: string }> = {
  ...
  business:    { ar: 'الإدارة والاتفاقات',  en: 'Business Admin',     workspace: '/business',   level: 'Admin' },
};
...
const rolePlans: {
  ...
  { role: 'developer',   missionAr: 'إدارة النظام والربط التقني والإصدارات.',                                  missionEn: 'Manage system, integrations, and releases.',                           kpiAr: 'لا أعطال حرجة',                  kpiEn: 'No critical blockers',                       handoverAr: 'يستلم البلاغات التقنية من التشغيل.',            handoverEn: 'Receives technical issues from operations.',                  dailyAr: ['إدارة Firebase', 'إعدادات النظام', 'إصلاح الأعطال', 'مراجعة الأزرار'],                                dailyEn: ['Manage Firebase', 'System settings', 'Fix bugs', 'Review buttons'],                                    weeklyAr: ['تجهيز APK', 'مراجعة الأداء', 'تحديث النسخة التجريبية'],       weeklyEn: ['Prepare APK', 'Review performance', 'Update preview build'],      limitsAr: ['لا يغير العقود والنسب', 'كلمة سره لا توحد مع الموظفين'],     limitsEn: ['No contracts/shares changes', 'Developer password is separate'] },
  { role: 'business',    missionAr: 'اعتماد العقود والنسب والتوسعة.',                                          missionEn: 'Approve contracts, shares, and expansion.',                            kpiAr: 'قرارات موثقة بعقد',              kpiEn: 'Documented decisions by contract',           handoverAr: 'يعتمد من التقارير المالية والتشغيلية.',         handoverEn: 'Approves from finance and operations reports.',               dailyAr: ['مراجعة التقارير', 'اعتماد القرارات العليا', 'متابعة المنصرفات للعلم'],                                dailyEn: ['Review reports', 'Approve high-level decisions', 'View costs for awareness'],                           weeklyAr: ['اعتماد المدن الجديدة', 'مراجعة الأرشيف التعاقدي', 'تحديث خطة التوسع'], weeklyEn: ['Approve new cities', 'Review contract archive', 'Update expansion plan'], limitsAr: ['لا تعديل مباشر للنسب', 'أي تغيير بعقد جديد'],               limitsEn: ['No direct share editing', 'Any change requires new contract'] },
];

const copy = {
  ar: { title: 'لوحات إعدادات الموظفين', sub: 'تنسيق احترافي لعمل الموظفين: صلاحيات، مهام، مؤشرات، تسليم، ومساحات عمل واضحة لكل دور.', back: 'العودة للإدارة والاتفاقات', portal: 'البوابة الموحدة', logout: 'تسجيل خروج', add: 'إضافة موظف', name: 'اسم الموظف', phone: 'رقم الهاتف', email: 'البريد الإلكتروني', role: 'الدور الوظيفي', shift: 'الوردية', internalNotes: 'ملاحظات داخلية للإدارة فقط', save: 'حفظ الموظف', active: 'نشط', paused: 'موقوف', employees: 'الموظفون', settings: 'بطاقة إعداد الموظف', tasks: 'بطاقات مهام الموظفين', copy: 'نسخ بيانات الدخول', copied: 'تم نسخ بيانات الدخول', empty: 'لا يوجد موظفون بعد.', denied: 'هذه الصفحة مخصصة لحساب الإدارة والاتفاقات فقط. سجّل الدخول من البوابة الموحدة.', toggle: 'English', apiMissing: 'لم يتم ربط NEXT_PUBLIC_API_URL بعد. سيتم استخدام حفظ تجريبي داخل المتصفح.', apiReady: 'متصل بالخادم', saved: 'تم حفظ الموظف وربطه بالخادم.', failed: 'تعذر الاتصال بالخادم. تم حفظ نسخة تجريبية محليًا.', hidden: 'مخفية بعد الحفظ', mission: 'المهمة', kpi: 'مؤشر الأداء', handover: 'التسليم والتصعيد', workspace: 'مساحة العمل', open: 'فتح اللوحة', level: 'المستوى', daily: 'مهام يومية', weekly: 'مهام أسبوعية', limits: 'حدود الصلاحية', defaultPass: 'كلمة السر الافتراضية 123456 — يجب تغييرها بعد أول دخول' },
  en: { title: 'Staff Settings Workspaces', sub: 'Professional coordination for staff work: permissions, tasks, KPIs, handover, and clear workspace per role.', back: 'Back to business', portal: 'Unified portal', logout: 'Logout', add: 'Add employee', name: 'Employee name', phone: 'Phone number', email: 'Email address', role: 'Job role', shift: 'Shift', internalNotes: 'Internal admin notes only', save: 'Save employee', active: 'Active', paused: 'Paused', employees: 'Employees', settings: 'Employee setup card', tasks: 'Employee task cards', copy: 'Copy login details', copied: 'Login details copied', empty: 'No employees yet.', denied: 'This page is only for the Business account. Log in from the unified portal.', toggle: 'العربية', apiMissing: 'NEXT_PUBLIC_API_URL is not connected. Local preview storage will be used.', apiReady: 'Connected to backend', saved: 'Employee saved and connected to backend.', failed: 'Backend unavailable. A local preview copy was saved.', hidden: 'Hidden after save', mission: 'Mission', kpi: 'KPI', handover: 'Handover', workspace: 'Workspace', open: 'Open panel', level: 'Level', daily: 'Daily tasks', weekly: 'Weekly tasks', limits: 'Access limits', defaultPass: 'Default password 123456 — must be changed after first login' },
};
...
function TaskList({ title, items }: { title: string; items: string[] }) {
  return (
    <div style={{ background: '#F7FAFC', border: '1px solid #E7EEF5', borderRadius: 16, padding: 12 }}>
      <b style={{ color: '#063B63' }}>{title}</b>
...
export default function StaffPage() {
  ...
  useEffect(() => {
    ...
    apiGet<Staff[]>('/api/staff', [])
      .then((data) => { setStaff(data); setNotice(isApiConfigured() ? t.apiReady : ''); })
      .catch(() => {
        const saved = localStorage.getItem('jnbk_staff');
        if (saved) setStaff(JSON.parse(saved));
        setNotice(t.apiMissing);
      });
  }, []);
  ...
  const activeCount = useMemo(() => staff.filter((x) => x.status === 'active').length, [staff]);
  const roleCount = useMemo(() => new Set(staff.map((x) => x.role)).size, [staff]);
  ...
  async function toggle(item: Staff) {
    ...
    try {
      ...
      setStaff(staff.map((x) => (x.id === item.id ? result.staff : x)));
      setSelected(result.staff);
    } catch {
      const updated = { ...item, status: next as 'active' | 'paused' };
      setStaff(staff.map((x) => (x.id === item.id ? updated : x)));
      setSelected(updated);
    }
  }
  ...
  return (
    <main dir={ar ? 'rtl' : 'ltr'} style={{ textAlign: ar ? 'right' : 'left' }}>
      ...
      <section className="grid settingsGrid">
        <div className="card"><p>{t.employees}</p><strong>{staff.length}</strong></div>
        <div className="card"><p>{t.active}</p><strong>{activeCount}</strong></div>
        <div className="card"><p>{t.role}</p><strong>{roleCount}</strong></div>
        <div className="card"><p>API</p><strong>{isApiConfigured() ? t.apiReady : 'Preview'}</strong></div>
      </section>
      ...
      <section className="panel">
        <h2>{t.add}</h2>
        <p className="muted">{t.defaultPass}</p>
        <form className="formGrid" onSubmit={submit}>
          ...
          <input name="shift" placeholder={t.shift} />
          <textarea name="internalNotes" placeholder={t.internalNotes} />
          <button className="primaryAction" type="submit">{t.save}</button>
        </form>
      </section>
      ...
      {selected ? (
        <section className="panel">
          ...
          <div className="table">
            <div className="row">
              <span>{t.name}</span><span>{selected.name}</span>
              <span>{t.role}</span><b>{ar ? roleLabels[selected.role].ar : roleLabels[selected.role].en}</b>
            </div>
            <div className="row">
              <span>Username</span><span>{selected.username}</span>
              <span>{t.workspace}</span><b>{roleLabels[selected.role].workspace}</b>
            </div>
            <div className="row">
              <span>{t.level}</span><b>{roleLabels[selected.role].level}</b>
              <span>Status</span><b>{selected.status}</b>
            </div>
          </div>
          <button type="button" className="primaryAction" style={{ marginTop: 12 }} onClick={() => copyLogin(selected)}>
            {t.copy}
          </button>
        </section>
      ) : null}
          ...
          {staff.length === 0 ? (
            ...
            staff.map((item) => (
              <div className="row" key={item.id}>
                <span>{item.name}<br /><small>{item.phone || item.email || ''}</small></span>
                <span>{ar ? roleLabels[item.role].ar : roleLabels[item.role].en}<br /><small>{item.shift || roleLabels[item.role].level}</small></span>
                <span>
                  {item.username}<br />
                  <a className="languageSwitch" style={{ color: '#063B63', background: '#F7FAFC', border: '1px solid #D9E2EC', marginTop: 6 }} href={`${roleLabels[item.role].workspace}?lang=${lang}`}>
                    {t.open}
                  </a>
                </span>
                <b>
                  <button type="button" className="primaryAction" style={{ padding: '8px 12px' }} onClick={() => toggle(item)}>
      ...
      <section className="panel">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(310px, 1fr))', gap: 16 }}>
          {rolePlans.map((plan) => (
            <article key={plan.role} style={{ background: 'white', border: '1px solid #E7EEF5', borderRadius: 24, padding: 18, boxShadow: '0 12px 30px rgba(6,59,99,.07)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                <div>
                  <p className="kicker" style={{ margin: 0 }}>{roleLabels[plan.role].level}</p>
                  <h3 style={{ color: '#063B63', margin: '6px 0 8px', fontSize: 22 }}>
                    {ar ? roleLabels[plan.role].ar : roleLabels[plan.role].en}
                  </h3>
                  <p className="muted" style={{ margin: 0 }}>{ar ? plan.missionAr : plan.missionEn}</p>
                </div>
                <a className="primaryAction" style={{ display: 'inline-flex', padding: '8px 12px' }} href={`${roleLabels[plan.role].workspace}?lang=${lang}`}>
                  {t.open}
                </a>
              </div>
              <div style={{ display: 'grid', gap: 10, marginTop: 14 }}>
                <div className="notice success" style={{ margin: 0 }}>
                  <b>{t.kpi}: </b>{ar ? plan.kpiAr : plan.kpiEn}
                </div>
                ...
                <div className="muted"><b>{t.handover}: </b>{ar ? plan.handoverAr : plan.handoverEn}</div>
              </div>
            </article>
          ))}
        </div>
