...
export default function DriverRegister() {
  ...
  async function submit() {
    if (!phone.trim() || !name.trim()) {
      Alert.alert('JUMBAK', lang === 'ar' ? 'أدخل الاسم ورقم الهاتف' : 'Enter name and phone');
      return;
    }
    setLoading(true);
    try {
      await registerDriver({ phone, name, cityId: city.id, vehicleTypeId: vehicle.id, plateNo, color, model });
      Alert.alert('JUMBAK', lang === 'ar' ? 'تم إرسال طلب السائق للمراجعة' : 'Driver request submitted for review');
      router.replace({ pathname: '/driver', params: { lang } });
    } catch {
      Alert.alert('JUMBAK', lang === 'ar' ? 'تعذر الاتصال بالخادم. سيتم فتح وضع السائق التجريبي.' : 'Backend offline. Opening driver preview.');
      router.replace({ pathname: '/driver', params: { lang } });
    } finally {
      setLoading(false);
    }
