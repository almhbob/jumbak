...
export type Country = { id: string; nameAr: string; nameEn: string; currency: string };
export type City = { id: string; countryId: string; nameAr: string; nameEn: string; zonesAr: string[]; zonesEn: string[] };
export type VehicleType = { id: string; nameAr: string; nameEn: string; baseFare: number; perKmFare: number; minimumFare: number };
...
const rufaaZoneNames = rufaaZones.map((z) => z.nameAr);

export const cities: City[] = [
  { id: 'rufaa',    countryId: 'sd', nameAr: 'رفاعة',  nameEn: 'Rufaa',   zonesAr: rufaaZoneNames, zonesEn: rufaaZoneNames },
...
export function estimateFare(distanceKm: number, vehicleTypeId?: string) {
  const v = findVehicleType(vehicleTypeId);
  return Math.max(Math.round(v.baseFare + distanceKm * v.perKmFare), v.minimumFare);
}
